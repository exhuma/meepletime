...
def _require_membership(db: Session, circle_id: uuid.UUID, user_id: uuid.UUID) -> CircleMembership:
    """Return the membership or raise HTTP 403 if the user is not a circle member."""
    membership = (
...
def _validate_date(local_date: date, circle: Circle, membership: CircleMembership) -> None:
    """Raise HTTP 400/403 if *local_date* is beyond the 3-month horizon or is a past date for a regular member."""
...
def get_availability(
    ...
    """Return all availability records in [start_date, end_date] for *circle_id*."""
...
def set_availability(
    ...
    """Create or update the current user's availability state for *local_date*. Write is idempotent for the same state."""
...
def delete_availability(
    ...
    """Delete the current user's availability for *local_date*, equivalent to setting state to empty."""
    circle = _get_circle_or_404(db, circle_id)
    membership = _require_membership(db, circle_id, current_user.id)
    _validate_date(local_date, circle, membership)

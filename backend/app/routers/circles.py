...
@router.post("/circles", response_model=CircleOut, status_code=status.HTTP_201_CREATED)
def create_circle(
...
@router.post("/circles/join", response_model=MembershipOut, status_code=status.HTTP_201_CREATED)
def join_circle(
    join_in: InviteJoin,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
    ...
    if pseudonym_taken:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Pseudonym already taken in this circle")

...
@router.patch("/circles/{circle_id}/members/{user_id}", response_model=MembershipOut)
def update_membership(
    ...
    if update_in.role is not None and not is_admin:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only owner/admin can change roles")
    ...
    if "pseudonym" in update_data:
        ...
        if pseudonym_taken:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Pseudonym already taken")
...
def remove_member(
    ...
    if target.role == MemberRole.owner:
        owner_count = (
            db.query(CircleMembership)
            .filter(CircleMembership.circle_id == circle_id, CircleMembership.role == MemberRole.owner)
            .count()
        )
        if owner_count <= 1:
            raise HTTPException(
